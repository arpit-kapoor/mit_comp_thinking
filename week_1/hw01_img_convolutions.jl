...
example_vector = [0.5, 0.4, 0.3, 0.2, 0.1, 0.0, 0.7, 0.8, 0.9, 0.6]
...
random_vect = rand(example_vector, 10) # replace this with your code!
...
function vecvec_to_matrix(vecvec)
	n, m = length(vecvec), length(vecvec[1])
	mat = zeros(Int64, n, m)
	for i in 1:n
		for j in 1:m
			mat[i, j] = vecvec[i][j]
...
function matrix_to_vecvec(matrix)
	n, m = size(matrix)
	vecvec = [[matrix[i,j] for j in 1:m] for i in 1:n]
	return vecvec
end
...
function mean_colors(image)
	H, W = size(image)
	vec = [image[i, j] for i in 1:H for j in 1:W]
	r = mean([pixel.r for pixel in vec])
	g = mean([pixel.g for pixel in vec])
	b = mean([pixel.b for pixel in vec])			
	return (r, g, b)
end
...
begin
	function quantize(x::Number)
		...
		return floor(x, digits=1, base=10)
	end
...
# ╔═╡ 91614ade-7e4b-11eb-03cb-c1d398d4e8c6
size(philip)
...
function blur_1D(v, l)
	...
	_v = copy(v)
	for i in 1:n
		_v[i] = mean([extend(_v, j) for j in i-l:i+l])
	end
	return _v
end
...
@bind l_box Slider(1:1:n, show_value=true)

# ╔═╡ 76f274bc-7e54-11eb-03f5-7f941ce66fc6
colored_line(v)

# ╔═╡ 8021d1fe-7e54-11eb-0918-51b6f6460243
...
function convolve_vector(v, k)
	...
	for i in 1:n
		conv_v[i] = sum([extend(v, j) for j in i-l:i+l] .* k)
	end
	
	return conv_v
end

# ╔═╡ 80745f6a-7e58-11eb-3ed3-1561081d8fc1
begin
	v2 = rand(10)
	k = [0.25, 0.5, 0.25]
end

# ╔═╡ 88cce682-7e58-11eb-1d9d-110687e0db37
colored_line(v2)

# ╔═╡ 90dbd416-7e58-11eb-0147-5bdad6844cc6
colored_line(k)

# ╔═╡ b0b92de0-7e58-11eb-15f9-9bfc6afd374b
colored_line(convolve_vector(v2, k))
...
begin 
	...
	function gaussian_kernel(n)
		σ = 1
		kernel = [gauss(x,σ) for x in ceil(-n/2):floor(n/2)]
...
@bind gaussian_kernel_size_1D Slider(1:1:10, show_value=true)# change this value, or turn me into a slider!

# ╔═╡ c6070800-7e64-11eb-174e-3dce1679770c
colored_line(random_vect)

# ╔═╡ a5c9c73e-7e5b-11eb-1539-cb1f685c032b
colored_line(gaussian_kernel(gaussian_kernel_size_1D))

# ╔═╡ 0ea63534-7e5d-11eb-11ca-83a438bf1f9f
colored_line(create_bar())
...
function extend_mat(M::AbstractMatrix, i, j)
	...
	if i in 1:num_rows && j in 1:num_columns
		ext_i, ext_j = i, j
	else
		ext_i = max(min(i, num_rows), 1)
		ext_j = max(min(j, num_columns), 1)
	end
	return copy(M[ext_i, ext_j])
end
...
# ╔═╡ f5d10736-7e67-11eb-0be0-5d870b4e7fb0
@bind i Slider(-1:7, show_value=true)

# ╔═╡ 28256a56-7e68-11eb-2871-33bf5ad30c97
@bind j Slider(-1:7, show_value=true)

# ╔═╡ 94a8424e-7e67-11eb-08d2-53fd4bdfa945
extend_mat(small_image, i, j)
...
# ╔═╡ 2ce4913a-7e67-11eb-3398-5320c6c6aeb4
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	for row in 1:num_rows
		for col in 1:num_columns
			patch = [extend_mat(M, i, j)  for j in col-l:col+l for i in row-l:row+l]
			patch = reshape(patch, size(K))
			conv_M[row, col] = sum(patch .* K)
		end
	end
...
# ╔═╡ b6fc9c8a-7e6e-11eb-0ee1-27e7584b5b79
...
# ╔═╡ 3c70ad18-7e77-11eb-0f04-6dc984cbefbd
md"""Select row index"""

# ╔═╡ 2a6f2ec8-7e72-11eb-093a-fb88f4246ef4
@bind row Slider(1:9, show_value=true)

# ╔═╡ 5151ee0e-7e77-11eb-2a5d-137824a6d164
md"""Select column index"""

# ╔═╡ 3ce5b626-7e72-11eb-2bcd-bdfb2d208cd2
@bind col Slider(1:9, show_value=true)

# ╔═╡ bae0a834-7e71-11eb-2c8d-d7ffe71f97ce
begin
	M = copy(test_image_with_border)
	num_rows, num_columns = size(M)
	l = (size(K_test)[1] - 1) ÷ 2

	patch = [extend_mat(M, i, j)  for j in col-l:col+l for i in row-l:row+l]
	patch = reshape(patch, size(K_test))
end

# ╔═╡ b890b47e-7e72-11eb-386d-5b22c10b7419
sum(patch .* K_test)

# ╔═╡ 6efb8302-7e77-11eb-3eb8-e3251356e160
md"""**Convolved Image**"""
...
# ╔═╡ bd380a52-7e76-11eb-04eb-5764e0dc32cb
K_test_2 = ones(3, 3) / 9 

# ╔═╡ e7f8b41a-ee25-11ea-287a-e75d33fbd98b
convolve_image(philip, K_test_2)
...
begin
	function gauss_2D(x, y, σ)
		return (1 / sqrt(2* π * σ^2)) * exp( -(x^2+y^2) / (2*σ^2) )
	end

	function gaussian_kernel_2D(n, σ)
		kernel = [gauss_2D(x, y, σ) for y in ceil(-n/2):floor(n/2) for x in ceil(-n/2):floor(n/2)]
		kernel = reshape(kernel, (n+1,n+1))
		kernel = kernel./sum(kernel)
		return kernel
	end
	
	function with_gaussian_blur(image, kernel_size, σ)
		kernel = gaussian_kernel_2D(kernel_size, σ)
...
# ╔═╡ 6739a3ca-7e79-11eb-28d7-791b00f7e19f
md"""**Sigma value**"""

# ╔═╡ a1c6d83c-7e79-11eb-0c54-07fbd18bfa39
@bind σ Slider(0.1:0.01:5, show_value=true)

# ╔═╡ c8b33e96-7e7a-11eb-2613-cbcb6b0a6e83
@bind kernel_size NumberField(1:50)
...
begin
	...
	function get_sobel_filter(α=1)
		a = [0.125 0.25 0.125] .* α
		b = [1 0 -1]
		G_x = reshape(a, 3, 1) * b
		G_y = reshape(b, 3, 1) * a
		return G_x, G_y
	...
	function with_sobel_edge_detect(image, α=1)
		G_x, G_y = get_sobel_filter(α)		
		image_x = convolve_image(image, G_x)
		image_y = convolve_image(image, G_y)
		image_sobel = sqrt.(image_x.^2 + image_y.^2)
		return image_sobel
	end
end

# ╔═╡ 48f485a6-7fd4-11eb-212f-e5d5f878992b
get_sobel_filter()

# ╔═╡ 70262796-7fd5-11eb-393f-01bc472db5df
@bind α Slider(1:1:10, show_value=true)
...
@bind gauss_raw_camera_data camera_input(;max_size=200)
...
# ╔═╡ 7f1dbab8-7e78-11eb-17e4-2d29071fc5e0
with_gaussian_blur(gauss_camera_image, kernel_size, σ)
...
Gray.(with_sobel_edge_detect(sobel_camera_image, α))
